...
Ext.define('Ecfa.ux.panel.upload.ItemGridPanel', {
	...
	/**
	 * Loads the internal store with the supplied queue items. Convert Ecfa.ux.panel.upload.Item to grid record
	 * 
	 * @param {Array}
	 *            items
	 */
	loadQueueItems : function(items) {
		...
		for (i = 0; i < items.length; i++) {
			// console.log('items getStatus', items[i].getStatus());
			data.push({
	...
	statusRenderer : function(value, metaData, record, rowIndex, colIndex, store) {
		...
		// console.log('prg status', record.get('progress'), value);
		if (record.get('progress') == 100 && value == Ecfa.ux.panel.upload.Item.STATUS_UPLOADING) {
			return Locale.getMsg('view.transfer.file.status.processing');
		}

		if (value == Ecfa.ux.panel.upload.Item.STATUS_SKIP) {
			return '<a href="javascript:void(0)"  data-qtip=' + Locale.getMsg('view.transfer.file.status.skip.tip', 4, Ecfa.Config.FTP_SERVER) + '>'
					+ Locale.getMsg('view.transfer.file.status.skip') + '</a>'
		}
