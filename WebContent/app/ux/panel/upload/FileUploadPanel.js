...
Ext.define('Ecfa.ux.panel.upload.FileUploadPanel', {
	...
	initComponent : function() {
		...
		me.grid = Ext.create('Ecfa.ux.panel.upload.ItemGridPanel', {
			queue : this.queue,
		...
		me.uploadManager.on('itemuploadsuccess', me.onItemUploadSuccess, me);
		me.uploadManager.on('itemuploadfailure', me.onItemUploadFailure, me);

		me.statusBar = Ext.create('Ecfa.ux.panel.upload.StatusBar', {
			dock : 'bottom',
		...
		Ext.apply(me, {
			...
			uploading : false,
			items : [ me.grid ],
			dockedItems : [ me.getTopToolbarConfig(), me.statusBar ]
		});
		...
		addDomEventListener(window, 'beforeunload', function(e) {
			console.log('onBeforeunload uploadActive', me.uploadManager.uploadActive);
			if (me.uploadManager.uploadActive) {
